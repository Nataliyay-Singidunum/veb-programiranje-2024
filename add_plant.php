...
// priprema podataka da se kasnije kroz for each izvuku sve opcije za dodavanje
...
<html>
    ...
    <main>
        <section id="content">
            <div class="mirza-medium" id="add_plant">
                <h1>Add a new plant</h1>
    <form action="ins_plant.php" method="POST">
        ...
        <div>
            ...
            <select name="plant_family_name" id="plant_family_name">
                <?php foreach ($plant_families as $row) { // za svaki red u plant families izvrsavamo petlju kroz koju dodajemo opcije?>
                    <option value="<?php echo $row['plant_family_id'] // vrednost koja ide u bazu?>"><?php echo $row['plant_family'] // ispis selekcije?></option>
                <?php } ?>
            </select>
        </div>
        ...
        <div>
            ...
            <select name="plant_size" id="plant_size">
                <?php foreach ($plant_sizes as $row) { ?>
                    <option value="<?php echo $row['plant_size_id'] ?>"><?php echo $row['plant_size'] ?></option>
                <?php } ?>
            </select>
        </div>
        ...
        <div>
            ...
            <select name="plant_characteristic" id="plant_characteristic">
                <?php foreach ($plant_characteristics as $row) { ?>
                    <option value="<?php echo $row['plant_characteristic_id'] ?>"><?php echo $row['plant_characteristic'] ?></option>
                <?php } ?>
            </select>
        </div>
